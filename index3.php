					...
					<div class="content-page-form">
						<form method="POST" action="registration.php">
							<div class="form-group">
								<label title for="name">Name</label>
								<div class="controls">
									<input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" id="name" name="name" placeholder="name">
								</div>
							</div>
							<div class="form-group">
								<label title for="surname">Surname</label>
								<div class="controls">
									<input type="text" autocomplete="off" autocorrect="off" autocapitalize="off" id="surname" name="surname" placeholder="surname">
								</div>
							</div>
							<div class="form-group">
								<label title for="email">Email</label>
								<div class="controls">
									<input type="email" autocomplete="off" autocorrect="off" autocapitalize="off" id="email" name="email" placeholder="email">
									<label>After E-Mail must have @ and as least 6 symbols.</label>
								</div>
							</div>
							<div class="form-group">
								<label title for="password">Enter the password</label>
								<div class="controls">
									<input type="password" autocomplete="off" autocorrect="off" autocapitalize="off" id="password" name="password" placeholder="password">
									<label>After E-Mail must have @ and as least 6 symbols.</label>	
								</div>
							</div>
							<input type="submit" id="submit" class="btn btn-submit" value="Register">
                      ...
                      $("#submit").click(function(){
                        ...
                        $.ajax({
                          ...
                          success: function(response){
                            if (response=="") {
                              $(location).attr('href',"index4.php");
                            }else{
                              $("#result").html(response).css({color:"brown",fontWeight:"bold"});
                            }
                          }
                        });
                        return false;
